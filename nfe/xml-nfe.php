...
<?php
    ...
    $totaldesconto = $sales['total_discont'];
    $totalcomdesconto = $totalvendas - $totaldesconto;
    //Monta DV 65
    $cUF = $ufIbge;    
    $aamm = nfce(date('y/m'));     
    $cnpj = nfce(CNPJ);     
    ...
    */
    ...
    fwrite($ponteiro, "<natOp>$natureza</natOp>");
    ...
    fwrite($ponteiro, "<tpNF>$tipoNota</tpNF>");
    ...
    fwrite($ponteiro, "<finNFe>$finalidade</finNFe>");
...
if(!empty($complementocliente))    fwrite($ponteiro, "<xCpl>$complementocliente</xCpl>");
 ...
 $soma_icms = 0;
   ...
   $soma_confins = 0;
    ...
    $soma_icms_total += icms($subtotal,$icms);
    ...
    fwrite($ponteiro, "<qCom>".float4($quantity)."</qCom>");
    fwrite($ponteiro, "<vUnCom>".float4($preco)."</vUnCom>");
    fwrite($ponteiro, "<vProd>".float2($subtotal)."</vProd>");
    fwrite($ponteiro, "<cEANTrib/>");
    
    fwrite($ponteiro, "<uTrib>$uTrib</uTrib>");
    fwrite($ponteiro, "<qTrib>".float4($quantity)."</qTrib>");
    fwrite($ponteiro, "<vUnTrib>".float4($preco)."</vUnTrib>");
    ...
    $claculoicmsnota = $soma_icms_total;//icms($totalvendas,$soma_icms);
    ...
    fwrite($ponteiro, "<vDesc>$totaldesconto</vDesc>");
    ...
    fwrite($ponteiro, "<vNF>$totalcomdesconto</vNF>");
    fwrite($ponteiro, "</ICMSTot>");
    fwrite($ponteiro, "</total>");
