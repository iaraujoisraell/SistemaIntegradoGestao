...
define('EMPRESA', utf8_decode($empresa_nome));	
define('CNPJ',  $empresa_cnpj);
define('INSCRICAOESTADUAL',  $empresa_ie);
define('EMPRESA_ENDERECO', utf8_decode($empresa_endereco));	
define('EMPRESA_NUMERO', $empresa_numero);	
define('EMPRESA_BAIRRO', utf8_encode($empresa_bairro));	
define('EMPRESA_COMPLEMENTO', utf8_decode($empresa_complemento));	
define('EMPRESA_CIDADE',  utf8_decode($empresa_cidade));
...
<?php
    ...
    $nomecliente = nfce(utf8_encode($cliente['name']));
    $ieCliente = nfce(utf8_encode($cliente['cf1']));
    $indicacaoIeCliente = nfce(utf8_encode($cliente['indicacaoIeCliente']));
    $cpfcliente = nfce($cliente['vat_no']);
    $enderecocliente = nfce(utf8_encode($cliente['address']));
    $numerocliente = nfce(utf8_decode($cliente['cf2']));
    $complementocliente = nfce(utf8_decode($cliente['cf3']));
    $bairrocliente = nfce(utf8_encode($cliente['cf5']));
    $codigoMunicipioCliente = $cliente['codigoMunicipio'];
    $cidadecliente = nfce(utf8_decode($cliente['city']));
    $estadocliente = nfce(utf8_decode($cliente['state']));
    ...
    $endereco = utf8_encode(nfce(EMPRESA_ENDERECO));
    $numero = nfce(EMPRESA_NUMERO);
    $bairro = utf8_encode(nfce(EMPRESA_BAIRRO));
    $complemento = utf8_encode(nfce(EMPRESA_COMPLEMENTO));
   ...
   $soma_confins = 0;
    ...
    $nomeproduto = utf8_encode(nfce($produtobling['product_name']));
    $detalhes_produto = decode_html($prds['product_details']);
    ...
    fwrite($ponteiro, "<vICMS>$icmsproduto</vICMS>");
    fwrite($ponteiro, "</ICMS00>");
    fwrite($ponteiro, "</ICMS>");
    ...
    ?>
    <script>
    document.location.href = ("integracaoAgil.php?nfe=nfce<?php echo $chave; ?>.xml&idEmpresa=<?php echo $idEmpresa;?>&idVenda=<?php echo $sales["id"];?>&num=<?php echo $num2; ?>");
    </script>
    <?php
}//if empty(xml)
?>